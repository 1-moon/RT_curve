...
int main()
{
	...
	for (int j = image_height - 1; j >= 0; j--) {
		...
		for (int i = 0; i < image_width; i++) {
			...
			auto t = hit_sphere(centre, 0.5, origin, vec);
			color pixel_color;
			// we find our point and the normal and then normalize normal 
			if (t > 0.0) {
				point3 p(origin + t * vec);
				Vec3 unit = unit_vector(p - centre);
				// make them range from 0~1 as we did earlier
				pixel_color = 0.5*color(unit.x()+1, unit.y() + 1, unit.z() + 1);
			}
			else {	
