...
public:
    ...
    bool TestIntersection(const Ray& r, Interval ray_t, Hit_record& rec) const override {
        ...
        if (!object->TestIntersection(offset_r, ray_t, rec))
            return false;
...
public:
    ...
    bool TestIntersection(const Ray& r, Interval ray_t, Hit_record& rec) const override {
        ...
        if (!object->TestIntersection(rotated_r, ray_t, rec))
            return false;

        // Change the intersection point from object space to world space
        auto p = rec.int_p;
        p[0] = cos_theta * rec.int_p[0] + sin_theta * rec.int_p[2];
        p[2] = -sin_theta * rec.int_p[0] + cos_theta * rec.int_p[2];
